...
namespace MBCPLUS_DAEMON
{
    ...
    {        
        ...
        private String m_pk;
        private String m_tc_pk;
        private String m_clip_pk;
        private String m_program_seq_pk;
        private String m_profileid;
        private String m_metahub_YN;
        private String m_transcode_YN;
        private String m_pathfilename;
        private String m_encset;
        private String m_encid;        
        ...
        public void GetFullurl(CdnInfo cdninfo, String ext, out String full_url, out String rtmp_url, out String hls_url)
        {
            String strFTPid = "";
            full_url = "";
            rtmp_url = "";
            ...
            JObject metaJson = new JObject(
                            new JProperty("api_request",
                                new JObject(
                                    ...
                                    new JProperty("file_path", m_pathfilename))));
        ...
        void Run()
        {
            ...
            {    
                ...
                {   
                    ...
                    {
                        m_pk = r["callback_pk"].ToString();
                        m_tc_pk = r["tc_pk"].ToString();
                        m_program_seq_pk = r["program_seq_pk"].ToString();
                        m_clip_pk = r["clip_pk"].ToString();
                        m_profileid = r["profileid"].ToString();
                        m_metahub_YN = r["metahub_YN"].ToString();
                        m_transcode_YN = r["transcode_YN"].ToString();
                        status = r["status"].ToString();
                        String ftppath = r["ftppath"].ToString();
                        ...
                        m_pathfilename = r["pathfilename"].ToString();
                        m_encid = r["encid"].ToString();
                        m_encset = r["encset"].ToString();

                        if (m_pathfilename.Length > 1)
                        {
                            m_pathfilename = m_pathfilename.Substring(1);
                        }
                        connPool.ConnectionOpen();
                        m_sql = String.Format("UPDATE TB_CALLBACK SET starttime = CURRENT_TIMESTAMP(), status = 'Running' WHERE callback_pk = '{0}'", m_pk);
                        //Running 으로 변경
                        ...
                        GetFullurl(cdninfo, Path.GetExtension(apiFtpPath), out full_url, out rtmp_url, out hls_url);
                        ...
                        mapper.UpdateCallbackURL(m_pk, full_url, rtmp_url, hls_url);
                        
                        log.logging(full_url + " encID : " + m_encid);

                        //if (m_encid == "notranscoding" || m_encid == "720p1M" || m_encid == "720p2M" || m_encset == "[copy]")
                        if (m_encid == "720p1M" || m_encid == "720p2M")
                        {                            
                            if (String.Equals(Path.GetExtension(full_url).ToLower(), ".mp4"))
                            {
                                ...
                                if (m_encid == "720p1M" || m_encid == "720p2M")
                                {
                                    ...
                                    mapper.InsertCLIPINFO(gid, cid, typeid, hls_url);
                                }
                                ...
                                foreach (DataRow r_cnt in ds_cnt.Tables[0].Rows)
                                {
                                    ...
                                    frmMain.WriteLogThread(String.Format(@"[CDNService] Callback cnt : {0}, clip_pk : {1}", callback_cnt, m_clip_pk));
                                    log.logging(String.Format(@"Callback cnt : {0}, clip_pk : {1}", callback_cnt, m_clip_pk));
                                        
                                    if (n_callback % 2 == 0 || (m_transcode_YN == "N" && n_callback >= 1) )
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            foreach (DataRow r_ftp in ds_ftp.Tables[0].Rows)
                                            {
                                                ...
                                                // 준비된 XML을 각 고객사에 전송
                                                frmMain.WriteLogThread("[CDNService] Ready to send : " + m_clip_pk + " target : " + map["ftp_target"].ToString());                                                
                                                    ...
                                                    foreach (String customer in ftp_target)
                                                    {
                                                        if (customer != "1" && customer != "2")
                                                        {
                                                            ...
                                                            m_sql = String.Format(@"UPDATE
                                                                        ...
                                                                        WHERE clip_pk = '{0}'
                                                                        AND customer_id = '{1}'
                                                                        AND type = 'XML'", m_clip_pk, customer);
                                                            cmd = new MySqlCommand(m_sql, connPool.getConnection());
                                                            cmd.ExecuteNonQuery();
                                                            connPool.ConnectionClose();
                                                        }
